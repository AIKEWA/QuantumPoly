 ...
 * @module lib/integrity/metrics/consent
...
export { ConsentEventType, ConsentCategory };
export type { ConsentLedgerEntry, ConsentMetrics };
...
export function aggregateConsentMetrics(
  ...
  categoryMetrics.essential.rate =
    totalUsers > 0 ? (categoryMetrics.essential.optIn / totalUsers) * 100 : 0;
  categoryMetrics.analytics.rate =
    totalUsers > 0 ? (categoryMetrics.analytics.optIn / totalUsers) * 100 : 0;
  categoryMetrics.performance.rate =
    totalUsers > 0 ? (categoryMetrics.performance.optIn / totalUsers) * 100 : 0;
