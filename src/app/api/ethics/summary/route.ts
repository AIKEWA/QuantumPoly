...
export async function GET() {
  try {
    ...
    const blocks = entries.map((entry) => 
      entry.blockId || entry.block_id || entry.id || entry.entry_id || "unknown"
    );
