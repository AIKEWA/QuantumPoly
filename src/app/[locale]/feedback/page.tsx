/**
 * @fileoverview Public Feedback Page (Block 10.6)
...
export default function FeedbackPage() {
  ...
  const maxChars = 2000;
  const charCountColor = charCount > maxChars ? 'text-red-600' : charCount > maxChars * 0.9 ? 'text-yellow-600' : 'text-gray-600';
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {state === 'error' && Object.keys(errors).length > 0 && (
        ...
        >
          <h2 id="error-summary-title" className="text-sm font-semibold text-red-800 dark:text-red-200">
            {t('validationError')}
          </h2>
      ...
      {state === 'success' && result?.id && (
        ...
        >
          ...
          <p className="mt-4 text-sm text-green-700 dark:text-green-300">
            {t('nextSteps')} <a href="mailto:governance@quantumpoly.ai" className="font-semibold underline hover:no-underline">governance@quantumpoly.ai</a>
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          <legend className="text-sm font-semibold text-gray-700 dark:text-gray-300">
            {t('topicLabel')} <span className="text-red-600" aria-label="required">*</span>
          </legend>
        ...
        <div>
          <label htmlFor="message" className="block text-sm font-semibold text-gray-700 dark:text-gray-300">
            {t('messageLabel')} <span className="text-red-600" aria-label="required">*</span>
          </label>
          <textarea
            id="message"
            name="message"
        ...
        <div className="space-y-3">
          <div className="flex items-start">
            ...
            />
            <label htmlFor="consent-contact" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
              {t('consentLabel')}
            </label>
          </div>
          ...
          {consentContact && (
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('emailLabel')} <span className="text-red-600" aria-label="required">*</span>
              </label>
              <input
                type="email"
                id="email"
