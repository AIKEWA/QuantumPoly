...
class ReactionAnalyzer {
    ...
    static evaluateResponse(persona, artifact) {
        ...
        if (persona.traits.tech_literacy === 'high')
            baseComprehension += 20;
        if (persona.traits.tech_literacy === 'low')
            baseComprehension -= 20;
        // Cap at 0-100
        ...
        if (persona.traits.tech_literacy === 'low')
            friction *= 1.5;
        if (persona.traits.stress_threshold === 'low')
            friction *= 1.2;
        ...
        const hasPrivacyTriggers = privacyTriggerWords.some((word) => artifact.content.toLowerCase().includes(word));
...
exports.ReactionAnalyzer = ReactionAnalyzer;
