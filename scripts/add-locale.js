...
function parseArgs() {
    ...
    if (!/^[a-z]{2}(-[A-Z]{2})?$/.test(locale)) {
        console.error(`${colors.red}Error: Invalid locale code format. Use lowercase 2-letter code (e.g., 'en', 'es')${colors.reset}\n`);
        return null;
    }
 ...
 */
function markForTranslation(value) {
 ...
 */
function copyTranslationFiles(localeCode) {
 ...
 */
function updateI18nConfig(options) {
    ...
    if (content.includes(`'${options.locale}'`)) {
        console.log(`  ${colors.yellow}⚠${colors.reset} Locale '${options.locale}' already exists in i18n.ts`);
        return;
    }
    // Update locales array
    content = content.replace(/(export const locales = \[)([^\]]+)(\] as const;)/, (match, start, locales, end) => {
    ...
    // Update localeLabels
    content = content.replace(/(export const localeLabels: Record<Locale, string> = \{)([^}]+)(\};)/, (match, start, labels, end) => {
    ...
    if (options.direction === 'rtl' && content.includes('localeDirections')) {
        content = content.replace(/(export const localeDirections: Record<[^>]+> = \{)([^}]+)(\};)/, (match, start, directions, end) => {
            const updatedDirections = directions.trim() + `,\n  ${options.locale}: '${options.direction}',`;
            return `${start}\n  ${updatedDirections}\n${end}`;
        });
    }
 ...
 */
function updateMiddleware(localeCode) {
    let content = fs.readFileSync(MIDDLEWARE_FILE, 'utf-8');
    // Update matcher pattern
    content = content.replace(/(matcher: \[['"]\/['"], ['"]\/)(\([^)]+\))(\/:\w+\*['"]\])/, (match, start, locales, end) => {
        const localeList = locales
...
function printInstructions(options) {
    ...
    console.log(`   Edit files in ${colors.cyan}src/locales/${options.locale}/${colors.reset}`);
    console.log(`   Replace all ${colors.red}[NEEDS_TRANSLATION]${colors.reset} markers with actual translations\n`);
