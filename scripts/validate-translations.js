...
function loadTranslationFile(locale, file) {
    ...
    try {
        ...
        console.error(`${colors.red}✗ Error parsing ${locale}/${file}: ${error instanceof Error ? error.message : String(error)}${colors.reset}`);
        process.exit(1);
    }
}
/**
 ...
 */
function comparePlaceholders(sourceText, targetText) {
 ...
 */
function validateLocale(locale, sourceData, targetData) {
    const errors = [];
 ...
 */
function printResults(errors, locales) {
    ...
    for (const locale of locales) {
        ...
        if (localeErrors.length === 0) {
            console.log(`${colors.green}✓ ${locale.toUpperCase()}: All validations passed${colors.reset}`);
        }
        else {
            hasErrors = true;
            console.log(`${colors.red}✗ ${locale.toUpperCase()}: ${localeErrors.length} error(s) found${colors.reset}\n`);
            ...
            for (const [file, fileErrors] of errorsByFile) {
                ...
                for (const error of fileErrors) {
                    const icon = error.type === 'missing_key'
                        ? '⚠ Missing'
    ...
    console.log(`${colors.cyan}═══════════════════════════════════════════${colors.reset}\n`);
    if (hasErrors) {
        console.log(`${colors.red}Validation failed with ${totalErrors} total error(s).${colors.reset}\n`);
        console.log(`${colors.yellow}Please fix the issues above and run validation again.${colors.reset}\n`);
    }
    else {
        console.log(`${colors.green}✓ All translations are valid and consistent!${colors.reset}\n`);
    }
}
/**
...
function main() {
    ...
    console.log(`${colors.blue}Found locales: ${locales.join(', ')}${colors.reset}\n`);
    if (!locales.includes(SOURCE_LOCALE)) {
        console.error(`${colors.red}Error: Source locale '${SOURCE_LOCALE}' not found!${colors.reset}`);
        process.exit(1);
    }
    ...
    console.log(`${colors.blue}Loaded ${sourceFiles.length} file(s) from ${SOURCE_LOCALE} locale${colors.reset}\n`);
    ...
    const hasErrors = Array.from(errors.values()).some((localeErrors) => localeErrors.length > 0);
