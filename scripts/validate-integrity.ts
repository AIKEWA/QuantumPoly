...
const LOCALES_ROOT = path.join(ROOT_DIR, 'src/locales');
const SOURCE_LOCALE = 'en';
const TARGET_LOCALES = ['de', 'tr']; // As requested: en, de, tr
const CONTENT_DIRS = ['content', 'docs'];
const REPORT_PATH = path.join(ROOT_DIR, 'reports', 'FPP15_LOCALE_VALIDATION.md');

// --- Types ---
interface KeyIssue {
    type: 'missing' | 'extra' | 'mismatch_type';
    path: string;
    key: string;
}
...
interface LinkIssue {
    file: string;
    link: string;
    type: 'broken' | 'absolute' | 'orphaned'; // orphaned not fully implemented yet but can be inferred if needed
    message: string;
}

interface ValidationReport {
    localeIssues: FileReport[];
    linkIssues: LinkIssue[];
}

const REPORT_DATA: ValidationReport = {
    localeIssues: [],
    linkIssues: []
};
...
function syncObject(source: any, target: any, keyPath: string = '', issues: KeyIssue[], lang: string): any {
...
async function validateLocales() {
    ...
    const sourceFiles = await globby(`**/*.json`, { cwd: path.join(LOCALES_ROOT, SOURCE_LOCALE) });
    ...
    for (const targetLang of TARGET_LOCALES) {
        ...
        for (const file of sourceFiles) {
            const sourcePath = path.join(LOCALES_ROOT, SOURCE_LOCALE, file);
            ...
            if (issues.length > 0) {
                ...
                fs.writeFileSync(targetPath, JSON.stringify(syncedContent, null, 2) + '\n');
            }
        }
    }
}
...
async function validateLinks() {
    ...
    const files = await globby(patterns, { cwd: ROOT_DIR });
    ...
    for (const file of files) {
        ...
        while ((match = linkRegex.exec(content)) !== null) {
            ...
            if (!linkPath) continue; // Just an anchor like '#'
            ...
            if (linkPath.startsWith('/')) {
                ...
                targetPath = path.resolve(path.dirname(absoluteFilePath), linkPath);
            }

            // Check existence
            if (!fs.existsSync(targetPath)) {
                 // Try adding .md or .tsx extensions if missing
                 if (fs.existsSync(targetPath + '.md')) targetPath += '.md';
                 else if (fs.existsSync(targetPath + '.tsx')) targetPath += '.tsx';
                 else if (fs.existsSync(targetPath + '.ts')) targetPath += '.ts';
                 else {
                     // Still missing
