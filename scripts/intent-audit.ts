#!/usr/bin/env ts-node
/**
 * intent-audit.ts - Audit tool for detecting silent successes in Next.js application
 * 
...
function printRouteTree(node: any, prefix: string, isLast: boolean) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const typedValue = value as { _hasPage?: boolean, _hasLayout?: boolean };
    const childHasContent = typedValue._hasPage || typedValue._hasLayout;
    let routeLabel = key;
    ...
    if (childHasContent) {
      console.log(`${label}${chalk.green(routeLabel)} ${
        typedValue._hasPage && typedValue._hasLayout
